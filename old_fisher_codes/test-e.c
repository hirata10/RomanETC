...
void get_bao_fisher_matrix2(double **F, char FileName[], COSMOPARAM *p, int flag) {
  ...
  for(i=0; i<N; i++) {
    ...
    fprintf(stderr, "%5.3lf %12.5le %12.5le %6.4lf %6.4lf %6.3lf\n",
...
#if 0
  fp = fopen("/newman/user2/chirata/krause/chris_Cl.dat", "w");
  for(i=0; i<start_gg; i++)
    fprintf(fp, "%19.12le\n", Cl[i]);
  fclose(fp);
  fp = fopen("/newman/user2/chirata/krause/chris_Cov.dat", "w");
  for(i=0; i<start_gg; i++)
    for(j=0; j<start_gg; j++)
      fprintf(fp, "%19.12le\n", ClCov[i][j]);
  fclose(fp);
  sym_matrix_invert(ClCov,ClCov,start_gg);
  fp = fopen("/newman/user2/chirata/krause/chris_invCov.dat", "w");
  for(i=0; i<start_gg; i++)
    for(j=0; j<start_gg; j++)
      fprintf(fp, "%19.12le\n", ClCov[i][j]);
  fclose(fp);
...
int ik;
...
#if 0
  printf("sigma8 = %9.6lf\n", get_linsigma(&p,0.,8./p.h));
#endif

  fp = fopen("es_distances_Z8.dat", "w");
  for(i=2;i<40;i++) {
    z = 0.2*i+0.1;
    fprintf(fp, "%2d %6.4f %6.4f %9.3lf %9.3lf %12.6le\n", i-2, 0.2*i, 0.2*i+0.2, get_DRC(0.2*i,&p), get_DRC(0.2*i+0.2,&p),
      4.5e-7*(get_DRC(0.2*i+0.2,&p)-get_DRC(0.2*i,&p))*(1+z)*(1+z)
    );
  }
  fclose(fp);
  fp = fopen("es_ps_Z8.dat", "w");
  for(i=2;i<40;i++) {
    z = 0.2*i+0.1;
    D = get_DRC(0.2*i+0.1,&p);
    dD = get_DRC(0.2*i+0.2,&p) - get_DRC(0.2*i,&p);
    for(j=20;j<=6000;j++) {
      k = (j+0.5)/D;
      get_Delta2k(&p, z, k, 1, 1, D2);      
      get_Delta2kNL(&p, z, k, 1, 1, D2NL);      
      D2[0] *= 2.*M_PI*M_PI/k/k/k;
      D2NL[0] *= 2.*M_PI*M_PI/k/k/k;
      fprintf(fp, "%2d %4d %12.6le %12.6le\n",
        i-2, j,
        j*(j+1.)/(2.*M_PI) * D2  [0] /dD/D/D,
        j*(j+1.)/(2.*M_PI) * D2NL[0] /dD/D/D
      );
    }
  }
  fclose(fp);
...
#if 0
  for(k=0.01;k<0.201;k+=0.01) {
    alpha = DNeh_trf_mpc(&p,k) * DNeh_trf_mpc(&p,k) * pow(k,p.ns);
  ...
  Ly = 200/p.h;
  Lz = 200/p.h;
  for(kx=1e-4; kx<10; kx*=exp(dlnk)) {
    printf("[kx=%12.5le]\n", kx);
    for(kr=1e-4; kr<10; kr*=exp(dlnk)) {
      k = sqrt(kx*kx+kr*kr);
      get_Delta2k(&p, 0.2, k, 1, 1, &Del2);
      for(dphi=0.5; dphi<10; dphi+=1) {
        ky = kr*cos(dphi*M_PI/2./10.);
        kz = kr*sin(dphi*M_PI/2./10.);
        T = sin(Lz*kz)/(Lz*kz) * sin(Lx*kx)/(Lx*kx) * sin(Ly*ky)/(Ly*ky);
        var += Del2 * T * T * kx * kr * kr * dlnk * dlnk / 10. / (k*k*k);
      }
    }
  }
  printf("var = %9.6lf\n", var);
...
#if 0
  for(k=0.001; k<0.0095; k+=.001) {
    get_Delta2k(&p, 0.0, k, 0.0, 1, D2);
    printf("%12.5le %12.5le\n", k, D2[0]*(2.*M_PI*M_PI)/(k*k*k));
  }
  for(k=0.01; k<0.095; k+=.01) {
    get_Delta2k(&p, 0.0, k, 0.0, 1, D2);
    printf("%12.5le %12.5le\n", k, D2[0]*(2.*M_PI*M_PI)/(k*k*k));
  }
  for(k=0.1; k<0.95; k+=.1) {
    get_Delta2k(&p, 0.0, k, 0.0, 1, D2);
    printf("%12.5le %12.5le\n", k, D2[0]*(2.*M_PI*M_PI)/(k*k*k));
  }
  for(k=1; k<9.5; k+=1) {
...
#if 0
  get_Delta2k(&p, 2.23, 1e-4, 0.00115, 12000, D2);
  get_Delta2kNL(&p, 2.23, 1e-4, 0.00115, 12000, D2NL);
#if 0
  xi = xinl = 0;
  r = pow(6.0e12/2.775e11/(4.*M_PI)*3., 1./3.) / p.h;
  for(i=0; i<12000; i++) {
      k = 1e-4 * exp(0.00115*i);
      kr = k*r;
      T = kr>1e-4? 3.*(sin(kr)-kr*cos(kr))/(kr*kr): 1.;
      xi += D2[i] * T * T;
      xinl += D2[i] * T * T * log(k);
  }
  printf("ln keff = %8.4lf\n", xinl/xi);
  return(0);
#endif
  for(r=2.0/p.h; r<4.65/p.h; r+=0.2/p.h) {
    xi = xinl = 0;
    for(i=0; i<12000; i++) {
      k = 1e-4 * exp(0.00115*i);
      dphi = 0.00115*k*r/2.;
...
#if 0
  get_Delta2k(&p, 0.1, 1e-4, 0.00115, 12000, D2);
  get_Delta2kNL(&p, 0.1, 1e-4, 0.00115, 12000, D2NL);
  for(r=1.0/p.h; r<100.1/p.h; r*=pow(10.,0.025)) {
  xi = xinl = 0;
  for(i=0; i<12000; i++) {
    k = 1e-4 * exp(0.00115*i);
    dphi = 0.00115*k*r/2.;
    J0 = 0.;
    for(j=0;j<4000;j++) {
      T = 0.00025*M_PI*(j+0.5);
      J0 += cos(k*r*cos(T));
    }
    J0 *= 0.00025;
    xi += 0.00115 * D2[i] * J0 * sin(dphi)/dphi * M_PI/k;
    xinl += 0.00115 * D2NL[i] * J0 * sin(dphi)/dphi * M_PI/k;
//    if (i%50==0) printf("i=%5ld k=%12.5le Delta^2=%12.5le xi=%12.5le\n", i, k, D2NL[i], xi);
  }
  printf("%9.5lf %9.5lf %9.5lf\n", r*p.h, xi*p.h, xinl*p.h);
  }
  return(0);
#endif

#if 0
  for(M=3.77976325e-3; M<3.8e5; M*=pow(10.,0.05)) {
    printf("%12.5le %12.5le %8.5lf\n",
      M * 2.775e11 * p.omh2 * p.h,
      get_fM(&p,M,0.1) * pow(p.h,-3),
      get_bM(&p,M,0.1)
    );
  }
  return(0);
  ...
  nc = nbc = 0.;
  i = 0;
  for(M=3.77976325e5*pow(10.,0.2); M>3.7e-3; M/=pow(10.,0.01)) {
    nc +=  get_fM(&p,M*pow(10.,0.005),0.1) * pow(p.h,-3) * 0.01*log(10);
    nbc += get_fM(&p,M*pow(10.,0.005),0.1) * pow(p.h,-3) * get_bM(&p,M*pow(10.,0.005),0.1) * 0.01*log(10);
    if (i%5==0 && i>19)
    printf("%12.5le %12.5le %8.5lf\n",
      M * 2.775e11 * p.omh2 * p.h, nc, nbc/nc
    );
    i++;
  }
  return(0);
...
#if 0
  xi = 0;
  get_Delta2kNL(&p, 0.3, 1e-4, 0.00115, 12000, D2NL);
  r = 20./p.h;
  for(i=0; i<12000; i++) {
    k = 1e-4 * exp(0.00115*i);
    dphi = 0.00115*k*r/2.;
    ...
    for(j=0;j<4000;j++) {
      T = 0.00025*M_PI*(j+0.5);
      J2 += cos(2*T)*cos(k*r*cos(T));
    }
    J2 *= -0.00025;
    xi += 0.00115 * D2NL[i] * J2 * sin(dphi)/dphi * M_PI/k;
//    if (i%50==0) printf("i=%5ld k=%12.5le Delta^2=%12.5le xi=%12.5le\n", i, k, D2NL[i], xi);
  }
  printf("wp @ 20 Mpc/h = %12.5le Mpc/h\n", xi*p.h);
  return(0);
#endif
...
// get_sn_fisher_matrix(F, argv[1], &p, 0x8);
